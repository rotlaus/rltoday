...
static BOOL CretaeList2(HWND hListView)
{
...
static BOOL CALLBACK SelectFileDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendDlgItemMessage(hDlg, IDC_STATIC_FILE, WM_SETTEXT, 0, (LPARAM)TEXT("File"));
		SendDlgItemMessage(hDlg, IDCANCEL, WM_SETTEXT, 0, (LPARAM)TEXT("Cancel"));
...
static BOOL CALLBACK SelectDirectoryDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	SHINITDLGINFO shidi;
	SHFILEINFO shfi;
	LVCOLUMN lvc;
...
//		SetWindowText(hDlg, TEXT("Select file"));
	...
	case WM_SHOWFILELIST:
		...
		CretaeList2(GetDlgItem(hDlg, IDC_LIST_FILE));
		ListView_SortItems(GetDlgItem(hDlg, IDC_LIST_FILE), CompareFunc, 1);
...
BOOL SelectDirectory(HWND hDlg, HINSTANCE hInst, TCHAR *fname, TCHAR *ret)
{
	...
	rc = DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_DIALOG_SELECTDIRECTORY), hDlg, SelectDirectoryDlgProc, (LPARAM)fname);
	if(rc == TRUE){
